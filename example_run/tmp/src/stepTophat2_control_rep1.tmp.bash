...
python /usr/local/share/dolphin_tools/src/jobStatus.py -f None -u docker -k kGwU8f4YSSbSTNsMt3c57DRiqtqBFG -s stepTophat2 -t dbSetStartTime -o /export/genome_data/example_run -j stepTophat2_control_rep1 -m 2 -n $JOB_NUM
   retval=$?
   if [ $retval -ne 0 ]; then
     exit 66
   fi
...
python /usr/local/share/dolphin_tools/src/jobStatus.py -f None -u docker -k kGwU8f4YSSbSTNsMt3c57DRiqtqBFG -s stepTophat2 -t dbSetEndTime -o /export/genome_data/example_run -j stepTophat2_control_rep1 -m 3 -n $JOB_NUM
    retval=$?
   if [ $retval -ne 0 ]; then
     exit 66
   fi
  ...
  echo failed
python /usr/local/share/dolphin_tools/src/jobStatus.py -f None -u docker -k kGwU8f4YSSbSTNsMt3c57DRiqtqBFG -s stepTophat2 -t dbSetEndTime -o /export/genome_data/example_run -j stepTophat2_control_rep1 -m 0 -n $JOB_NUM
    retval=$?
   if [ $retval -ne 0 ]; then
     exit 66
   fi
